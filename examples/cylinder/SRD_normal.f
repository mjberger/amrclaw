...
c     nCount is size of neighborhood, numHoods is number of merged nhoods each cells is in
     ...
     .               mptr,noDiagonals,ffluxlen,gfluxlen)
...
c
c             only give to cell that has you as its neighbor
...
c     2 cells to a side for a merging neighborhood
     ...
     &                         xlow,ylow,dx,dy,k)
         ...
         if (ar(k) .gt. areaMin) then
           ! nothing needs to be merged - set offsets to itself
           svi(k) = 0
           svj(k) = 0
           go to 10
         endif
...
!   initialize array with  most common case, overwritten below as needed
     ...
     .                        ycentMerge(i,j),xlow,ylow,dx,dy,k)
         ...
         vmerge = ar(k)  ! to start with, note it is diff variable than above, weighted by numhoods
         call getCellCentroid(lstgrd,i,j,xc,yc,xlow,ylow,dx,dy,k)
         xcent = xc/numHoods(i,j)   ! initial centroid
         ycent = yc/numHoods(i,j)
